...
def denue(path_resource: PathResource) -> gpd.GeoDataFrame:
    fpath = Path(path_resource.jobs_path) / "denue_2023_estimaciones.csv"
    df = pd.read_csv(
        fpath,
        usecols=("codigo_act", "longitud", "latitud", "num_empleos_esperados"),
...
def denue_split(
    ...
    return (
        denue_base.sjoin(df_agebs[["geometry"]], how="inner", predicate="within")
